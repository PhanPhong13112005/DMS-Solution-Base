  ...
  <div class="w-full text-left bg-background text-text-main relative">
    ...
    <div 
      ...
      :class="[
        ...
        statusType === 'success' ? 'bg-primary text-background border-white/10' : 'bg-amber-500 text-white border-amber-500'
      ]"
    >
    ...
    <section class="bg-[#A5A58D] text-white py-12 px-6 relative overflow-hidden">
      ...
      <div class="max-w-7xl mx-auto relative z-10 flex flex-col justify-center">
        ...
        <h1 class="text-3xl md:text-5xl font-serif font-light uppercase mb-3 text-white tracking-tight leading-none">
          Liên hệ & <span class="italic font-normal text-secondary">Phản hồi</span>
        </h1>
    ...
    <section class="py-16 bg-background">
      <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
          <div class="bg-white rounded-[32px] border border-border p-6 flex items-start gap-4 shadow-xs text-left hover:border-primary transition-all duration-300">
            <div class="w-12 h-12 bg-background rounded-2xl flex items-center justify-center text-primary shrink-0">
              <MapPin class="w-5.5 h-5.5" />
            </div>
            <div>
              <h4 class="font-bold text-text-main text-sm mb-1">Địa chỉ Ký túc xá</h4>
          ...
          <div class="bg-white rounded-[32px] border border-border p-6 flex items-start gap-4 shadow-xs text-left hover:border-primary transition-all duration-300">
            ...
            <div>
              <h4 class="font-bold text-text-main text-sm mb-1">Hotline giải đáp</h4>
              <p class="text-xs text-text-muted font-light font-mono leading-relaxed">
          ...
          <div class="bg-white rounded-[32px] border border-border p-6 flex items-start gap-4 shadow-xs text-left hover:border-primary transition-all duration-300">
            ...
            <div>
              <h4 class="font-bold text-text-main text-sm mb-1">Hòm thư trực tuyến</h4>
              <p class="text-xs text-text-muted font-light font-mono leading-relaxed">
                ktx@dainam.edu.vn
              </p>
            </div>
          ...
          <div class="bg-white rounded-[32px] border border-border overflow-hidden shadow-xs aspect-video relative">
            <img 
            ...
            />
            <div class="absolute inset-0 bg-gradient-to-t from-primary/80 to-transparent flex items-end p-4 text-white">
              <div class="text-left">
                <span class="text-[10px] uppercase font-bold tracking-wider text-secondary">Vị trí thực tế</span>
          ...
          <div class="bg-white rounded-[32px] border border-border p-8 shadow-xs">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-4 text-left">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1">
                  <label class="text-xs font-semibold text-text-main">Họ và tên <span class="text-secondary">*</span></label>
                  <input 
                    type="text"
                    required
                    v-model="name"
                    placeholder="Nguyễn Văn A"
                    class="w-full bg-background border border-border focus:border-primary rounded-2xl px-4 py-3 text-sm outline-none transition-all"
                  />
                </div>
                <div class="space-y-1">
                  <label class="text-xs font-semibold text-text-main">Mã sinh viên (MSSV)</label>
                  <input 
                    type="text"
                    v-model="studentId"
              ...
              <div class="space-y-1">
                <label class="text-xs font-semibold text-text-main">Thư điện tử Email <span class="text-secondary">*</span></label>
                <input 
                  type="email"
                  required
                  v-model="email"
              ...
              <div class="space-y-1">
                <label class="text-xs font-semibold text-text-main">Nội dung câu hỏi / Góp ý <span class="text-secondary">*</span></label>
                <textarea 
                  required
                  rows="5"
                  v-model="msg"
                  placeholder="Điền thắc mắc hoặc góp ý chi tiết về dịch vụ phòng, điện nước hoặc xử lý thủ tục..."
                  class="w-full bg-background border border-border focus:border-primary rounded-2xl px-4 py-3 text-sm outline-none resize-none transition-all"
                ></textarea>
              </div>
              ...
              <button 
                type="submit"
                :disabled="submitted"
                class="w-full md:w-auto px-8 py-3.5 bg-primary hover:bg-primary-hover disabled:bg-slate-300 text-white font-bold text-xs md:text-sm rounded-full shadow-md shadow-primary/10 transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
